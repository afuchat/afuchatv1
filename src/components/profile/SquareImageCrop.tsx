...
export const SquareImageCrop = ({ imageFile, open, onOpenChange, onSave }: SquareImageCropProps) => {
  ...
  const drawCanvas = () => {
    ...
    // Create square crop preview (show what will be kept)
    ctx.save();
    ctx.globalCompositeOperation = 'destination-in';
    ctx.fillRect(0, 0, size, size);
    ctx.restore();

    // Draw square border with rounded corners
    ctx.save();
  ...
  const handleSave = () => {
    ...
    // Apply square crop (no circular mask)
    // The image is already cropped to the square canvas bounds
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
          ...
          <div className="flex justify-center">
            <div className="relative">
              <canvas
                ref={canvasRef}
                className="rounded-lg cursor-move shadow-2xl border-4 border-primary/20"
