...
import { MessageCircle, Heart, Pencil, Repeat2, ArrowLeft, Share, Languages } from 'lucide-react';
...
const PostDetail = () => {
  ...
  const organizeReplies = (allReplies: Reply[]): Reply[] => {
    ...
    return rootReplies.sort((a, b) => (a.is_pinned === b.is_pinned ? new Date(b.created_at).getTime() - new Date(a.created_at).getTime() : a.is_pinned ? -1 : 1));
  };
  ...
  const fetchData = useCallback(async () => {
    ...
    if (repliesData) {
      const mapped = repliesData.map((r: any) => ({
        ...r,
        author: r.profiles
      }));
  ...
  const handleLike = async () => {
    ...
    try {
      if (wasLiked) await supabase.from('post_acknowledgments').delete().eq('post_id', post.id).eq('user_id', user.id);
      else await supabase.from('post_acknowledgments').upsert({ post_id: post.id, user_id: user.id });
    } catch {
      setIsLiked(wasLiked);
    } finally {
      setIsLiking(false);
  ...
  const handleTranslatePost = async () => {
    if (!post || isTranslatingPost) return;
  ...
  return (
    <main className="min-h-screen bg-background border-x border-border max-w-2xl mx-auto flex flex-col">
      {/* Sticky Header */}
      <nav className="sticky top-0 z-30 bg-background/95 backdrop-blur-md border-b border-border flex items-center gap-4 px-4 py-3">
      ...
      <div className="flex-1 overflow-y-auto">
        <article ref={postRef} className="p-4 border-b border-border">
          ...
          <div className="text-xl leading-normal mb-4">
          ...
          <Button variant="ghost" size="sm" className="text-primary mb-4" onClick={handleTranslatePost} disabled={isTranslatingPost}>
            <Languages className="mr-2 h-4 w-4" /> {translatedPost ? 'Original' : 'Translate'}
          </Button>

          {post.post_images && post.post_images.length > 0 && <ImageCarousel images={post.post_images.map(i => i.image_url)} />}
          
          <div className="text-muted-foreground text-sm py-4 border-b border-border">
          ...
          <div className="flex justify-around py-2">
            ...
            <Button variant="ghost" size="sm" onClick={() => navigator.clipboard.writeText(window.location.href).then(() => toast.success('Copied!'))}><Share className="h-5 w-5" /></Button>
          </div>
        </article>
